...
import { isCallExpression, isIdentifier, isMemberExpression } from './zzz-ts-estree';
// import {
//   BooleanLiteral,
//   ClassDeclaration,
//   createNodeArray,
...
//   return isClassDeclaration(klass) && klass.name ? klass.name.text : undefined;
// };
...
// export const getDecoratorArgument = (decorator: Decorator): ObjectLiteralExpression | undefined => {
...
//   const property = properties.find(prop => !!(prop.name && isIdentifier(prop.name) && prop.name.text === name));
...
// export const getComponentDecorator = (node: ClassDeclaration): Decorator | undefined => getDecorator(node, 'Component');

// export const getDirectiveDecorator = (node: ClassDeclaration): Decorator | undefined => getDecorator(node, 'Directive');

// export const getInjectableDecorator = (node: ClassDeclaration): Decorator | undefined => getDecorator(node, 'Injectable');

// export const getNgModuleDecorator = (node: ClassDeclaration): Decorator | undefined => getDecorator(node, 'NgModule');

export const getPipeDecorator = (
  node: TSESTree.ClassDeclaration,
): TSESTree.Decorator | undefined => getDecorator(node, 'Pipe');

export const getSymbolName = (expression: TSESTree.ExpressionWithTypeArguments): string => {
  const { expression: childExpression } = expression;

  // TODO: Investigate "as any"
  return isMemberExpression(childExpression) ? (childExpression.property as any).name : (childExpression as any).name;
};
...
export const getDeclaredInterfaces = (node: TSESTree.ClassDeclaration): TSESTree.ExpressionWithTypeArguments[] => {
  return node.implements || [];
};

export const getDeclaredInterfaceNames = (node: TSESTree.ClassDeclaration): string[] => getDeclaredInterfaces(node).map(getSymbolName);

export const getDeclaredInterfaceName = (node: TSESTree.ClassDeclaration, value: string): string | undefined =>
...
//   value.replace(unicodeWordMatch, val => val[0].toUpperCase() + val.slice(1).toLowerCase());
